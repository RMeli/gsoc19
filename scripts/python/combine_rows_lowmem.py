"""
Combine protein sequence distances and ligand similarities between all pairs of targets
in a single pickle object.

This script try keep a low memory usage and use vectorized operations where possible.

Input FILES have the following format:

    TARGET1 TARGET2 DISTANCE SIMILARITY
...
targets = np.loadtxt(files[0], usecols=1, dtype="U4")
...
assert (
    int(round(np.sum(dlsim[dlsim >= 0]))) - int(round(np.sum(dlsim[dlsim < 0])))
    == n_targets
)
print("done")
